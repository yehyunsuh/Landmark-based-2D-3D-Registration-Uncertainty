...
def train_model(args, model, device, train_loader, val_loader, optimizer, loss_fn):
    model.train()
    total_loss = 0
    ...
    for batch_idx, (images, masks, _, _) in enumerate(tqdm(train_loader, desc="Training")):
        ...
        if batch_idx % 10 == 0 and batch_idx > 0:
            val_loss, dists, mean_dice, _, _, _ = evaluate_model_while_training(model, device, val_loader)
            mean_dist = torch.nanmean(dists).item()
            if args.wandb:
                log_results(loss.item(), val_loss, mean_dist, mean_dice)
...
def evaluate_model_while_training(model, device, val_loader):
    model.eval()
    total_loss = 0
    all_pred_coords = []
    all_gt_coords = []
    ...
    return avg_loss, dists, mean_dice, None, None, None
...
def train(args, model, device):
    ...
    for epoch in range(args.epochs):
        ...
        train_loss = train_model(
            args, model, device, train_loader, val_loader, optimizer, loss_fn
        )
        ...
        ctpel_masked_path = sorted(glob(f'{args.ctpel_preprocessed_dir}/**/*_masked.nii.gz', recursive=True), key=lambda x: os.path.basename(x))[args.case_number]
        case_id = os.path.basename(ctpel_masked_path).split('_')[0]
        if mean_dist < best_mean_error:
            best_mean_error = mean_dist
            torch.save(model.state_dict(), f"{args.model_weight_dir}/{args.wandb_name}_{case_id}_label{args.landmark_to_predict}_dist.pth")
            print("✅ Saved new best model based on mean distance!")

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save(model.state_dict(), f"{args.model_weight_dir}/{args.wandb_name}_{case_id}_label{args.landmark_to_predict}_loss.pth")
